...
class GCNModel(nn.Module):
    def __init__(self, num_comb_features, gcn_hidden_dims, fc_hidden_dims, output_dim, pooling_method,
        ...
        # Create GCN layers with optional batch normalization, dropout, and skip connections
        ...
        in_channels = num_comb_features
        for j, hidden_dim in enumerate(gcn_hidden_dims):
            self.gcn_layers.append(GCNConv(in_channels, hidden_dim))
            if gcn_skip_connections[j]:
                self.gcn_skip_connections.append(nn.Linear(in_channels, hidden_dim))
            else:
                self.gcn_skip_connections.append(None)
        ...
        for j, hidden_dim in enumerate(fc_hidden_dims):
            self.fc_layers.append(nn.Linear(in_channels, hidden_dim))
        ...
        self.classifier = nn.Linear(fc_hidden_dims[-1], output_dim)
    ...
    def forward(self, data):
        # Process event-level features through GCN layers with optional batch normalization, dropout, activation, and skip connections
        x = data.x
        ...
        for i, gcn in enumerate(self.gcn_layers):
            ...
            mask = (x != -1).float()  # Recalculate mask based on the new x        
            # Apply the mask again after the GCN operation
            x = x * mask

            if self.gcn_skip_connections[i]:
                skip = self.gcn_skip_connections[i](skip)
                #if skip.shape[-1] != x.shape[-1]: # Apply adjustment if dimensions don't match
                    #skip = self.gcn_skip_connections[i](skip)
                x += skip  # Add skip connection
            if self.gcn_batch_norms[i]:
                x = self.gcn_batch_norms[i](x)            
            x = self.gcn_activation[i](x)
            if self.gcn_dropouts[i]:
                x = self.gcn_dropouts[i](x)        
            # Apply the mask again after activation and dropout
            x = x * mask
        ...
        for i, fc in enumerate(self.fc_layers):
            graph_emb = fc(graph_emb)
            if self.fc_batch_norms[i]:
                graph_emb = self.fc_batch_norms[i](graph_emb)
            graph_emb = self.fc_activation[i](graph_emb)
            if self.fc_dropouts[i]:
                graph_emb = self.fc_dropouts[i](graph_emb)
        
        # Classification
        out = self.classifier(graph_emb)
        return F.log_softmax(out, dim=1)

def objective(trial, model_save_folder, train_dataset, test_dataset, num_comb_features, output_dim, patience, epochs, device):
   
    # Define paths to save the best model
    #model_save_path = os.path.join('../output/model_hp/1levelGCN/', f'trial_{trial.number}.pth')
    ...
    gcn_skip_connections = [trial.suggest_categorical(f'gcn_skip_connections_{i}', [True, False]) for i in range(num_gcn_layers)]
    ...
    if lr_scheduler_name == 'StepLR':
        ...
        total_steps = len(train_loader)*(epochs + 10)
    ...
    model = GCNModel(num_comb_features=num_comb_features, 
        ...
        if epoch != 0: 
            if (test_accuracy > best_accuracy) or (test_accuracy == best_accuracy and val_loss_std_dev < best_std_dev) or (test_accuracy == best_accuracy and val_loss_std_dev == best_std_dev and test_loss < best_loss):
                ...
                torch.save({
                    ...
                    'num_comb_features': num_comb_features,
                    'gcn_hidden_dims': gcn_hidden_dims,
                    'fc_hidden_dims': fc_hidden_dims,
                    'output_dim': output_dim,
                    'pooling_method': pooling_method,
...
def load_model(model_class, path, device):
    ...
    model = model_class(
        num_comb_features=checkpoint['num_comb_features'], 
