...
def find_optimal_parameters(event, duration_col, 
    ...
    for i, (t_cut, q_bins) in enumerate(param_combinations):
        if i % 10 == 0:
            print(f"  Progress: {i}/{len(param_combinations)} - Current best: T_cut={best_params[0] if best_params else 'None'}, Q_bins={best_params[1] if best_params else 'None'}, Balance={best_balance:.1f}")
...
def smart_auto_binning_pipeline(event, duration_col, 
    ...
    if verbose:
        ...
        print(f"Small values (1-{pattern_analysis.get('detected_small_cutoff', 1)-1}) represent: {pattern_analysis.get('small_value_percentage', 0):.1f}% of data")
