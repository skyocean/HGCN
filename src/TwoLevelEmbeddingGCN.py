...
from torch_geometric.nn import GCNConv, global_mean_pool, GatedGraphConv, global_max_pool, global_add_pool
...
class EventSequenceEmbeddingGCNModel(nn.Module):
    def __init__(self, num_event_features, num_sequence_features, 
                 ...
                 gcn_dropout_rate, fc_dropout_rate, gcn_activation, fc_activation, gcn_skip_connections, 
                 num_embedding_features, gcn_hidden_dims_embedding, gcn_batch_norm_flag_embedding, gcn_momentum_embedding, gcn_eps_embedding,
                 gcn_dropout_flag_embedding, gcn_dropout_rate_embedding, gcn_activation_embedding,
                 gcn_hidden_dims_concat, gcn_batch_norm_flag_concat, gcn_momentum_concat, gcn_eps_concat,
                 gcn_dropout_flag_concat, gcn_dropout_rate_concat,gcn_activation_concat,
        ...
        super(EventSequenceEmbeddingGCNModel, self).__init__()
        
        self.embedding = nn.Embedding(num_embedding_features, embedding_dims)
        # Create GCN embedding layers with optional batch normalization, dropout
        ...
        for j, hidden_dim in enumerate(gcn_hidden_dims_embedding):
            self.gcn_layers_embedding.append(GCNConv(in_embedding, hidden_dim))
        ...
        for j, hidden_dim in enumerate(gcn_hidden_dims_concat):
            self.gcn_layers_concat.append(GCNConv(in_concat, hidden_dim))
...
def objective(trial, model_save_folder, train_dataset, test_dataset, num_event_features, num_embedding_features, num_sequence_features, output_dim, patience, epochs, device):
    ...
    model = EventSequenceEmbeddingGCNModel(num_event_features=num_event_features, 
