...
class EventSequenceGCNModel(nn.Module):
    def __init__(self, num_event_features, num_sequence_features, gcn_hidden_dims, fc_hidden_dims, output_dim, pooling_method,
        ...
        super(EventSequenceGCNModel, self).__init__()
        ...
        for j, hidden_dim in enumerate(gcn_hidden_dims):
            ...
            self.gcn_skip_connections.append(nn.Linear(in_channels, hidden_dim) if gcn_skip_connections[j] else None)
        ...
        # Create fully connected layers for sequence features with optional batch normalization and dropout
       ...
       # Create fully connected layers for combined features with optional batch normalization and dropout
        ...
        in_fc_concat = in_channels + in_features
        ...
        #self.classifier = nn.Linear(fc_hidden_dims[-1] + gcn_hidden_dims[-1], output_dim)
    ...
    def forward(self, data, sequence_features):
        ...
        for i, gcn in enumerate(self.gcn_layers):
            ...
            if self.gcn_skip_connections[i]:
                ...
                if skip.shape[-1] != x.shape[-1]:  # Ensure skip and x have the same shape
                   skip = nn.Linear(skip.shape[-1], x.shape[-1])(skip)
...
def objective(trial, model_save_folder, train_dataset, test_dataset, num_event_features, num_sequence_features, output_dim, patience, epochs, device):
    ...
    # hyperpareameters for fc concate
    ...
    model = EventSequenceGCNModel(num_event_features=num_event_features, 
