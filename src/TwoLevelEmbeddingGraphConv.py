...
class CustomDataset(Dataset):
    def __init__(self, event_features, embedding_features, sequence_features, y):
    ...
    def __getitem__(self, idx):
        return self.event_features[idx], self.embedding_features[idx],self.sequence_features[idx], self.y[idx]
...
def train(model, loader, optimizer, criterion, device, l1_lambda):
    ...
    correct = 0
    for batch in loader:
        event_data, embedding_features, sequence_features, labels = batch
        ...
        optimizer.zero_grad()
        output = model(event_data, embedding_features, sequence_features)
...
class EventSequenceEmbeddingGraphConvModel(nn.Module):
    def __init__(self, num_event_features, num_sequence_features, 
                 ...
                 num_embedding_features, gcn_hidden_dims_embedding, gcn_batch_norm_flag_embedding, gcn_momentum_embedding, gcn_eps_embedding,
                 gcn_dropout_flag_embedding, gcn_dropout_rate_embedding, gcn_activation_embedding, gcn_aggrs_embedding,
                 ...
                 fc_dropout_flag_concat, fc_dropout_rate_concat, fc_activation_concat, embedding_dims):
        
        super(EventSequenceEmbeddingGraphConvModel, self).__init__()
        
        self.embedding = nn.Embedding(num_embedding_features, embedding_dims)
        # Create GraphConv embedding layers with optional batch normalization, dropout
        self.gcn_layers_embedding = nn.ModuleList()
        self.gcn_batch_norms_embedding = nn.ModuleList()
        self.gcn_dropouts_embedding = nn.ModuleList()
        self.gcn_activation_embedding = nn.ModuleList()
        in_embedding = embedding_dims
        for j, hidden_dim in enumerate(gcn_hidden_dims_embedding):
            self.gcn_layers_embedding.append(GraphConv(in_embedding, hidden_dim, aggr=gcn_aggrs_embedding[j]))
            if gcn_batch_norm_flag_embedding[j]:
                self.gcn_batch_norms_embedding.append(nn.BatchNorm1d(hidden_dim, eps=gcn_eps_embedding[j], momentum=gcn_momentum_embedding[j]))
            else:
                self.gcn_batch_norms_embedding.append(None)
            self.gcn_activation_embedding.append(self._get_activation(gcn_activation_embedding[j]))
            if gcn_dropout_flag_embedding[j]:
                self.gcn_dropouts_embedding.append(nn.Dropout(gcn_dropout_rate_embedding[j]))
            else:
                self.gcn_dropouts_embedding.append(None)
            in_embedding = hidden_dim
                
        
        # Create GraphConv layers with optional batch normalization, dropout
        ...
        # Create GraphConv concate layers with optional batch normalization, dropout
        ...
        num_concat_features = + in_embedding + in_channels 
    ...
    def forward(self, data, embedding_features, sequence_features):
        
        d = embedding_features.x     
        d = self.embedding(d)
        d = d.squeeze(1) 
 
        for i, gcn in enumerate(self.gcn_layers_embedding):
            d = gcn(d, embedding_features.edge_index, edge_weight=embedding_features.edge_attr)
            if self.gcn_batch_norms_embedding[i]:
                d = self.gcn_batch_norms_embedding[i](d)
            d = self.gcn_activation_embedding[i](d)
            if self.gcn_dropouts_embedding[i]:
                d = self.gcn_dropouts_embedding[i](d)       
        
        
        # Process event-level features through GraphConv layers with optional batch normalization, dropout, activation
        f = data.x
        ...
        for i, gcn in enumerate(self.gcn_layers):
            ...
            # Apply the mask again after the GraphConv operation
            f = f * mask
...
def objective(trial, model_save_folder, train_dataset, test_dataset, num_event_features, num_embedding_features, num_sequence_features, output_dim, patience, epochs, device):
    ...
    embedding_dims = trial.suggest_int('embedding_dims', 10, 50)
    num_gcn_layers_embedding = trial.suggest_int('num_gcn_layers_embedding', 1, 5)
    gcn_hidden_dims_embedding = [trial.suggest_int(f'gcn_hidden_dim__embedding{i}', 32, 256) for i in range(num_gcn_layers_embedding)]
    ...
    gcn_batch_norm_flag_embedding = [trial.suggest_categorical(f'gcn_batch_norm_flag_embedding_{i}', [True, False]) for i in range(num_gcn_layers_embedding)]
    gcn_momentum_embedding = [trial.suggest_float(f'gcn_momentum_embedding_{i}', 0.01, 0.99) if gcn_batch_norm_flag_embedding[i] else 0 for i in range(num_gcn_layers_embedding)]
    gcn_eps_embedding = [trial.suggest_float(f'gcn_eps_embedding_{i}', 1e-5, 1e-2) if gcn_batch_norm_flag_embedding[i] else 0 for i in range(num_gcn_layers_embedding)]
    gcn_dropout_flag_embedding = [trial.suggest_categorical(f'gcn_dropout_flag_embedding_{i}', [True, False]) for i in range(num_gcn_layers_embedding)]
    gcn_dropout_rate_embedding = [trial.suggest_float(f'gcn_dropout_rate_embedding_{i}', 0.1, 0.5) if gcn_dropout_flag_embedding[i] else 0 for i in range (num_gcn_layers_embedding)]
    gcn_activation_embedding = [trial.suggest_categorical(f'gcn_activation_embedding_{i}', ['relu', 'leaky_relu', 'elu', 'tanh', 'softplus', 'gelu']) for i in range(num_gcn_layers_embedding)]
    gcn_aggrs_embedding = [trial.suggest_categorical(f'gcn_aggrs_embedding_{i}', ['add', 'mean', 'max']) for i in range(num_gcn_layers_embedding)]
    ...
    model = EventSequenceEmbeddingGraphConvModel(num_event_features=num_event_features, 
                                  ...
                                  gcn_batch_norm_flag_embedding=gcn_batch_norm_flag_embedding, 
                                  gcn_momentum_embedding=gcn_momentum_embedding, 
                                  gcn_eps_embedding=gcn_eps_embedding,
                                  gcn_dropout_flag_embedding=gcn_dropout_flag_embedding, 
                                  gcn_dropout_rate_embedding=gcn_dropout_rate_embedding, 
                                  gcn_activation_embedding=gcn_activation_embedding,
                                  gcn_aggrs_embedding=gcn_aggrs_embedding,
   ...
   # Select optimizer based on the trial suggestion
    ...
    for epoch in range(epochs):
        train_loss, train_accuracy = train(model, train_loader, optimizer, criterion, device, l1_lambda)
        test_loss, test_accuracy = evaluate(model, test_loader, criterion, device)
        print(f'Epoch {epoch+1}, Loss: {train_loss:.4f}, Accuracy: {train_accuracy:.4f}, Test Loss: {test_loss:.4f}, Test Accuracy: {test_accuracy:.4f}')
       
        # Step the scheduler if it's not ReduceLROnPlateau
        if lr_scheduler_name != 'ReduceLROnPlateau':
            scheduler.step()
        else:
            scheduler.step(test_accuracy)  # ReduceLROnPlateau requires a metric to monitor
        ...
        if epoch != 0:
            if (test_accuracy > best_accuracy) or (test_accuracy == best_accuracy and val_loss_std_dev < best_std_dev) or (test_accuracy == best_accuracy and val_loss_std_dev == best_std_dev and test_loss < best_loss):
...
def load_model(model_class, path, device):
    ...
    model = model_class(
        ...
        num_embedding_features=checkpoint['num_embedding_features'], 
        gcn_hidden_dims_embedding=checkpoint['gcn_hidden_dims_embedding'],                                   
        gcn_batch_norm_flag_embedding=checkpoint['gcn_batch_norm_flag_embedding'], 
        gcn_momentum_embedding=checkpoint['gcn_momentum_embedding']if checkpoint['gcn_batch_norm_flag_embedding'] else 0, 
        gcn_eps_embedding=checkpoint['gcn_eps_embedding'] if checkpoint['gcn_batch_norm_flag_embedding'] else 0,
        gcn_dropout_flag_embedding=checkpoint['gcn_dropout_flag_embedding'], 
        gcn_dropout_rate_embedding=checkpoint['gcn_dropout_rate_embedding'] if checkpoint['gcn_dropout_flag_embedding'] else 0, 
        gcn_activation_embedding=checkpoint['gcn_activation_embedding'],
        gcn_aggrs_embedding=checkpoint['gcn_aggrs_embedding'],
    ...
    return model, optimizer, loss_function, batch_size, l1_lambda, epoch, best_accuracy,best_loss, best_std_dev
